...
fn collect_needed_from_lexicon_snapshot(
    lexicon_snapshot_hash: &Hash32,
    snapshot: &LexiconSnapshotV1,
) -> SyncResult<Vec<Hash32>> {
    let mut out: Vec<Hash32> = Vec::with_capacity(1 + snapshot.entries.len());
    out.push(*lexicon_snapshot_hash);
...
/// Sync a LexiconSnapshotV1 and all referenced LexiconSegmentV1 artifacts from a remote store.
pub fn sync_lexicon_v1(
    ...
    // Ensure snapshot exists locally, then decode it to discover the segment closure.
    ensure_one(lexicon_snapshot_hash)?;
    let snap_b = local_store
        .get(lexicon_snapshot_hash)?
        .ok_or_else(|| SyncErrorV1::Proto("missing lexicon snapshot after fetch"))?;
    let snapshot = LexiconSnapshotV1::decode(&snap_b)
        .map_err(|_| SyncErrorV1::Proto("decode lexicon snapshot failed"))?;

    let needed = collect_needed_from_lexicon_snapshot(lexicon_snapshot_hash, &snapshot)?;
