...
impl IndexSegmentV1 {
    ...
    pub fn build_from_segment(seg_hash: Hash32, seg: &FrameSegmentV1) -> Result<IndexSegmentV1, IndexBuildError> {
    ...
    while i < flat.len() {
        ...
        let tf_sum = if tf_sum_u64 > (u32::MAX as u64) { u32::MAX } else { tf_sum_u64 as u32 };
    ...
    Ok(IndexSegmentV1 { seg_hash, source_id, row_count, terms, postings })
}
...
mod tests {
    ...
    fn index_segment_round_trip_encode_decode() {
        ...
        r1.terms.push(TermFreq { term: tid(10), tf: 2 });
        r1.terms.push(TermFreq { term: tid(11), tf: 1 });
        r1.doc_len = 3;

        let mut r2 = crate::frame::FrameRowV1::new(DocId(Id64(2)), src);
        r2.terms.push(TermFreq { term: tid(10), tf: 1 });
        r2.terms.push(TermFreq { term: tid(12), tf: 5 });
        r2.doc_len = 6;
    ...
    fn postings_iter_decodes_expected_rows() {
        ...
        let posts: Vec<PostingV1> = idx.postings_iter(pos).unwrap().map(|r| r.unwrap()).collect();
        assert_eq!(posts, vec![PostingV1 { row_ix: 0, tf: 2 }, PostingV1 { row_ix: 1, tf: 1 }]);
    }
}