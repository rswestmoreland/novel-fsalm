...
impl LexiconExpandLookupV1 {
    ...
    /// Lookup lemma ids for a lemma_key_id, writing results into `out`.
    ///
    /// This reuses the caller-provided buffer to avoid per-call allocations.
    pub fn lemma_ids_for_key_into(&self, key: LemmaKeyId, cap: usize, out: &mut Vec<LemmaId>) {
        out.clear();
        if cap == 0 {
            return;
        }
