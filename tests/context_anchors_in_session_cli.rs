...
#[test]
fn context_anchors_are_recorded_and_include_prior_terms() {
    let base = tmp_dir("context_anchors_are_recorded_and_include_prior_terms");
    let root = base.join("root");
    std::fs::create_dir_all(&root).unwrap();

    let bin = env!("CARGO_BIN_EXE_fsa_lm");

    // Minimal wiki ingest + index so retrieval has evidence.
    ...
    // Minimal wiktionary ingest to get a lexicon snapshot for lexicon-assisted anchors.
    ...
    // Run a short chat session with two turns.
...

    {
        ...
        stdin.write_all(b"Tell me about banana\n").unwrap();
        stdin.write_all(b"Why is it a fruit?\n").unwrap();
