...
use fsa_lm::index_store::{get_index_segment_v1_cached, put_index_segment_v1};
...
#[test]
fn index_segment_cached_get_avoids_second_store_read() {
    let dir = std::env::temp_dir()
        .join("fsa_lm_tests")
        .join("index_segment_cached_get_avoids_second_store_read");
    ...
    // Store segment to obtain a canonical seg_hash for index build.
    ...
    cfg.a1_ratio = 100;
    let mut cache: Cache2Q<Hash32, Arc<IndexSegmentV1>> = Cache2Q::new(cfg);

    let s1 = get_index_segment_v1_cached(&store, &mut cache, &idx_hash)
        .unwrap()
        .unwrap();
    assert_eq!(&*s1, &idx);
    ...
    let s2 = get_index_segment_v1_cached(&store, &mut cache, &idx_hash)
        .unwrap()
        .unwrap();
    assert_eq!(&*s2, &idx);
    assert_eq!(store.get_calls(), 1);
